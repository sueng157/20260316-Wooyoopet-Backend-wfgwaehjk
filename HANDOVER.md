...
- **현재 단계**: Phase 1 진행중 (1-5 테스트 데이터 삽입 미완, 1-1~1-4 완료)
...
| 1-5 | 테스트 데이터 삽입 | ❌ 미완 | SQL 10: CHECK 제약조건 불일치로 실패, 재작업 필요 |

**Phase 1 참고사항**:
- SQL 1~9는 Supabase SQL Editor에서 직접 실행하여 **DB에 이미 적용 완료** (로컬 파일 미보관)
- SQL 10 (테스트 데이터)은 상태값이 CHECK 제약조건과 불일치하여 실패 → 새 세션에서 DB 스키마를 직접 조회한 후 재생성 필요
- 핵심 테이블: members, kindergartens, pets, reservations, payments, refunds, settlement_infos, settlements
- 부가 테이블 (chats, messages, reviews, educations, contents, settings 등)은 테스트 데이터 미삽입
