...
- **현재 단계**: Phase 1 완료 (1-1~1-5 모두 완료), Phase 2 예정
...
현재 **백엔드 구축·호스팅 전환·기존 서버 정리** 작업이 진행 중입니다 (Phase 1~6).
...
| 1-5 | 테스트 데이터 삽입 | ✅ 완료 | SQL 10: `sql/10_test_data.sql` — 41개 테이블 중 39개에 데이터 삽입 |

**Phase 1 참고사항**:
- SQL 1~9는 Supabase SQL Editor에서 직접 실행하여 **DB에 이미 적용 완료** (로컬 파일 미보관)
- SQL 10 (테스트 데이터)은 `sql/10_test_data.sql`에 저장 — GraphQL introspection으로 전체 스키마(41개 테이블) 조회 후 CHECK 제약조건에 맞춰 재생성
- 핵심 8개 테이블: members(10명), kindergartens(3곳), pets(7마리), reservations(8건), payments(7건), refunds(2건), settlement_infos(2건), settlements(4건)
- 부가 테이블 포함: chat_rooms(3), chat_messages(10), reports(2), reviews(5), education_topics(3), checklists(2), pledges(1), banners(3), notices(3), faqs(5), terms(4), admin_accounts(3), feedbacks(3), audit_logs(5), noshow_records(2) 등 총 39개 테이블
...
| 순서 | 작업 | 상태 | 비고 |
|------|------|------|------|
| 2-1 | Supabase Auth 설정 | ⬜ 예정 | 관리자 이메일/비밀번호 인증 |
| 2-2 | 로그인 페이지 구현 | ⬜ 예정 | login.html + 로그인 JS |
| 2-3 | 세션 관리 | ⬜ 예정 | JWT 토큰 저장, 만료 시 리다이렉트 |
| 2-4 | 권한 체크 | ⬜ 예정 | 관리자 권한 등급별 메뉴 접근 제어 |
...
| 순서 | 작업 | 상태 | 비고 |
|------|------|------|------|
| 3-1 | Supabase 클라이언트 설정 | ⬜ 예정 | supabase-js CDN 로드, 클라이언트 초기화 |
| 3-2 | 대시보드 연동 (0번) | ⬜ 예정 | 통계 카드, 승인 대기, 매출 요약, 활동 로그 |
| 3-3 | 회원관리 연동 (1번) | ⬜ 예정 | 목록 조회·필터·검색·페이지네이션, 상세, 상태 변경 |
| 3-4 | 유치원관리 연동 (2번) | ⬜ 예정 | 목록+상세+기능 전체 |
| 3-5 | 반려동물관리 연동 (3번) | ⬜ 예정 | 목록+상세+기능 전체 |
| 3-6 | 돌봄예약관리 연동 (4번) | ⬜ 예정 | 목록+상세+기능 전체 |
| 3-7 | 결제관리 연동 (5번) | ⬜ 예정 | 결제내역·환불/위약금 탭 전체 |
| 3-8 | 정산관리 연동 (6번) | ⬜ 예정 | 정산정보·정산내역 탭 전체 |
| 3-9 | 채팅관리 연동 (7번) | ⬜ 예정 | 채팅내역·신고접수 탭 전체 |
| 3-10 | 후기관리 연동 (8번) | ⬜ 예정 | 보호자 후기·유치원 후기 탭 전체 |
| 3-11 | 교육관리 연동 (9번) | ⬜ 예정 | 교육 주제·체크리스트/서약서·이수현황 탭 전체 |
| 3-12 | 콘텐츠관리 연동 (10번) | ⬜ 예정 | 배너·공지사항·FAQ·약관 탭 전체 |
| 3-13 | 설정 연동 (11번) | ⬜ 예정 | 앱설정·관리자 계정·의견/피드백 탭 전체 |
| 3-14 | 공통 기능 연동 | ⬜ 예정 | 엑셀 다운로드, 감사 로그, 마스킹 전체보기 로그 |
