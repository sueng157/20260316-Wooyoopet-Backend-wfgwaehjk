...
[진행중] DB 연결 보완 및 UI 개선
  - 회원관리, 유치원관리, 반려동물관리 : 수정 완료(PR #59~#66)
  - 돌봄예약관리 : 수정 진행중(PR #70, #72)
  - 결제관리 ~ 설정 : 작업 예정
  ↓
...
- **현재 단계**: Phase 1~3 완료 (PR #48~#57) → DB 연결 보완 및 UI 개선 진행중 (PR #59~#66) → Phase 4 예정
...
| HTML | JS 참조 순서 |
|------|-------------|
| 전체 42페이지 (login 제외) | supabase-js CDN → supabase-client.js → auth.js → common.js → components.js → api.js |
...
| `contents.html`, `content-*.html` (9개) | ... → api.js → contents.js |
...
js/auth.js              365줄  (로그인/로그아웃, 세션 체크, 사이드바·헤더 프로필 동적 표시, 메뉴 접근 권한 제어)
js/common.js            141줄  (모달 시스템, 마스킹 토글, 소개글 토글, textarea→버튼 활성화)
js/components.js        224줄  (탭 전환, 전체선택 체크박스, 순서 화살표, 버전 검증, 글자수 카운터)
js/api.js               829줄  (Supabase CRUD 래퍼, 포매터, 배지, 페이지네이션, 엑셀, 감사로그, 마스킹, 권한)
...
js/reservations.js      461줄  (목록·상세, 직권취소, 노쇼 — PR #57에서 async/await 리팩터링)
js/payments.js          497줄  (결제/환불 2탭, 상세, 위약금면제 — PR #57에서 FK힌트 수정)
...
총 8,364줄  (Phase 3 완료 + DB 연결 보완·UI 개선 기준)
```
...
HTML/CSS/JS 프론트엔드 UI 구현이 모두 완료되었습니다 (42페이지, 15 CSS, 17 JS).
**백엔드 구축 Phase 1~3 완료** 후, 현재 **DB 연결 보완 및 UI 개선** 작업을 진행중입니다.
...
| reservations | 10 | 8개 상태 모두 포함 |
| payments | 10 | 결제완료 7 + 취소완료 3 (care_fee + walk_fee + pickup_fee = amount) |
| refunds | 4 | 환불완료 3 + 환불대기 1 |
...
- **DB ID 체계**: 모든 PK는 uuid 타입, 테스트 데이터는 고정 uuid 사용
  ...
  - reservations: `e0e0e0e0-0001~0010`
  - payments: `f0f0f0f0-0001~0010`
...
| 3-6 | 돌봄예약관리 연동 (4번) | ✅ 완료 | PR #54 — 목록(8상태필터), 상세(8영역), 직권취소, 노쇼 |
...
api.autoBadge(text)  // 커스텀 맵 인자 미지원 — STATUS_BADGE_MAP에 없으면 gray
```
...
| 조인 테이블 검색 | ✅ 해결됨 — `search_reservations` RPC로 보호자/유치원/반려동물 검색 구현 (PR #70) | - |
| autoBadge 커스텀 색상 | ✅ 해결됨 — `renderBadge()` 직접 호출로 변경 + `set_pet_size_class` 트리거로 DB 데이터 정정 (PR #70, #72) | - |
