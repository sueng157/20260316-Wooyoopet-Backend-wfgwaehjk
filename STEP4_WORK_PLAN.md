...
| # | 기능 | 유형 | 난이도 | 의존 |
|---|------|------|--------|------|
| 4-6 | `send-push` (FCM 푸시) | EF | 중 | — (기반 모듈) |
| 4-5 | `send-alimtalk` (카카오 알림톡) | EF | 중 | — |
| 4-10 | `app_get_blocked_list` (차단 목록) | RPC | 하 | — |
| 4-8 | `app_create_chat_room` (채팅방 생성) | RPC | 상 | — |
| 4-9 | `app_get_chat_rooms` (채팅방 목록) | RPC | 상 | 4-8 |
| 4-2 | `send-chat-message` (채팅 메시지) | EF | 상 | 4-6 |
| 4-1 | `inicis-callback` (결제 콜백) | EF | 상 | — |
| 4-3 | `create-reservation` (예약 생성) | EF | 상 | 4-6, 4-8 |
| 4-4 | `complete-care` (돌봄 완료) | EF | 중 | 4-6 |
| 4-7 | `scheduler` (자동 상태 변경) | EF | 상 | 4-6, 4-4 |
...
**R1 완료 기준**:
- `send-push`: FCM 멀티캐스트 발송 + 만료 토큰 정리 + `notifications` INSERT
- `send-alimtalk`: Supabase Auth SMS 훅 + 루나소프트 API 호출
...
**R2 완료 기준**:
- `app_create_chat_room`: SECURITY DEFINER + 중복 방 검사 + 나간 방 복원 + `chat_room_members` 2건 INSERT
- `app_get_chat_rooms`: 미읽음 카운트(`created_at` 타임스탬프 비교) + 상대방 프로필 JOIN + `reservation_count`
...
**R5 완료 기준**:
- 등원/하원 30분 전 알림
- 돌봄 시작/종료 시점 시스템 메시지 + Realtime
- `auto_complete_scheduled_at` 도달 시 자동 완료
- `scheduler_history` 기록
- pg_cron 또는 외부 cron 연동 가이드
...
|--------|--------|--------|--------|------|
| `chat_rooms` | ✅ 참여자만 | — | ✅ 참여자만 | `chat_room_members` EXISTS 체크 |
